...
import { artists, coPerformers, type Artist } from "../lib/data";
...
function Heart({ artistId, size = 22 }: { artistId: string; size?: number }) {
  ...
  return (
    <button
      ...
      className={`relative z-10 grid shrink-0 place-items-center transition ${following ? "text-text" : "text-text-faint hover:text-text"}`}
    >
      <svg viewBox="0 0 24 24" width={size} height={size} fill={following ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.7" className="block" aria-hidden>
...
function usePlayback(a: Artist) {
  ...
  const isCurrent = !!currentSong && a.songs.some((s) => s.id === currentSong.id);
  const rowPlaying = isCurrent && isPlaying;
  const toggle = () =>
    isCurrent ? setIsPlaying(!isPlaying) : first && play(first, a.songs);
  return { isCurrent, rowPlaying, toggle, play, first };
}
...
function CoPerformers({ a }: { a: Artist }) {
  ...
  const shortest = peers.reduce((s, p) => (p.name.length < s.name.length ? p : s));
  return (
...
function ClassicRow({ a }: { a: Artist }) {
  ...
  return (
    <li className="group relative flex h-12 items-center overflow-hidden rounded-xl bg-surface-2 pr-3 transition hover:bg-surface-3 sm:h-16 lg:h-24 lg:pr-6">
      <Link to={`/artist/${a.id}`} preload="hover" viewTransition aria-label={a.name} className="absolute inset-0" />
      <button
        onClick={toggle}
      ...
      <div className="pointer-events-none relative h-full w-36 shrink-0 overflow-hidden sm:w-48 lg:w-72" style={{ viewTransitionName: `img-${a.id}` }}>
      ...
      <div className="ml-4 flex min-w-0 flex-1 items-center gap-6 lg:ml-6 lg:gap-10">
        ...
        <div className="pointer-events-none min-w-0 flex-1">
          <p className={`truncate text-base font-semibold ${isCurrent ? "text-accent" : "text-text"}`} style={{ viewTransitionName: `name-${a.id}` }}>
            {a.name}
          </p>
          <p className="mt-0.5 truncate text-sm text-text-faint" style={{ viewTransitionName: `date-${a.id}` }}>
            <span className="text-text-dim">{a.distanceKm.toFixed(1)} km away</span>
          ...
          <div className="flex shrink-0 items-center gap-3">
            <div className="pointer-events-none hidden items-center gap-1.5 md:flex">
              {a.genres.slice(0, 3).map((g) => (
                <span key={g} className="rounded-full bg-surface-3 px-2 py-0.5 text-xs text-text-dim">
                  {g}
                </span>
              ))}
            </div>
...
function CompactRow({ a }: { a: Artist }) {
  const { isCurrent, rowPlaying, toggle } = usePlayback(a);
  return (
    <li className="group relative flex items-center gap-4 overflow-hidden rounded-lg bg-surface-2 px-2.5 py-2 transition hover:bg-surface-3">
      <Link to={`/artist/${a.id}`} preload="hover" viewTransition aria-label={a.name} className="absolute inset-0" />
      <button
        onClick={toggle}
        aria-label={rowPlaying ? `Pause ${a.name}` : `Play ${a.name}`}
        className="relative z-10 grid h-9 w-9 shrink-0 place-items-center text-text-dim transition hover:text-text"
      >
        <PlayPause playing={rowPlaying} className="h-5 w-5" />
      </button>
      <div className="pointer-events-none relative h-11 w-11 shrink-0 overflow-hidden rounded-md" style={{ viewTransitionName: `img-${a.id}` }}>
        <img src={a.photo} alt="" className="h-full w-full object-cover object-center" />
      </div>
      ...
      <div className="pointer-events-none flex min-w-0 flex-1 items-center gap-4 text-sm lg:gap-8">
        <span className={`min-w-0 flex-1 truncate font-medium ${isCurrent ? "text-accent" : "text-text"}`} style={{ viewTransitionName: `name-${a.id}` }}>
          {a.name}
        </span>
        <span className="hidden min-w-0 flex-1 truncate text-text-faint lg:block" style={{ viewTransitionName: `date-${a.id}` }}>
          {a.performing?.date}
        </span>
      ...
      <div className="flex shrink-0 items-center gap-3">
        <div className="pointer-events-none hidden w-48 items-center gap-1.5 overflow-hidden xl:flex">
          {a.genres.slice(0, 3).map((g) => (
            <span key={g} className="shrink-0 rounded-full bg-surface-3 px-2 py-0.5 text-xs text-text-dim">
              {g}
            </span>
          ))}
        </div>
...
// ── 2. Immersive: tall photo + hover-to-preview + genre pills ────────────────
function ImmersiveRow({ a }: { a: Artist }) {
  const { isCurrent, rowPlaying, toggle, play, first } = usePlayback(a);
  const timer = useRef<number | undefined>(undefined);
  return (
    <li
      className="group relative flex min-h-24 items-center gap-4 overflow-hidden rounded-xl bg-surface-2 pr-4 transition hover:bg-surface-3"
      onPointerEnter={(e) => {
        // preview-on-hover (mouse only) — discover by ear without a click
        if (e.pointerType === "mouse" && first)
          timer.current = window.setTimeout(() => play(first, a.songs), 500);
      }}
    ...
    >
      ...
      <div className="flex min-w-0 flex-[1.4] items-center gap-4 self-stretch">
        <div className="relative w-24 shrink-0 self-stretch overflow-hidden sm:w-44 lg:w-64" style={{ viewTransitionName: `img-${a.id}` }}>
          <img src={a.photo} alt="" className="pointer-events-none h-full w-full object-cover object-center" />
          <button
            onClick={toggle}
            aria-label={rowPlaying ? `Pause ${a.name}` : `Play ${a.name}`}
            className="absolute inset-0 z-10 grid place-items-center bg-black/35 opacity-0 transition group-hover:opacity-100"
          >
        ...
        <div className="pointer-events-none flex min-w-0 flex-1 flex-col justify-center gap-1 py-2">
          <p className={`text-lg font-semibold leading-tight ${isCurrent ? "text-accent" : "text-text"}`} style={{ viewTransitionName: `name-${a.id}` }}>
            {a.name}
          </p>
          <p className="text-sm leading-snug text-text-faint" style={{ viewTransitionName: `date-${a.id}` }}>
            <span className="text-text-dim">{a.distanceKm.toFixed(1)} km away</span>
            {a.performing && ` · ${a.performing.date} · ${a.performing.venue}`}
          </p>
        </div>
...
function GridCard({ a }: { a: Artist }) {
  const { isCurrent, rowPlaying, toggle } = usePlayback(a);
  return (
    <li className="group relative overflow-hidden rounded-xl bg-surface-2 transition hover:bg-surface-3">
      <Link to={`/artist/${a.id}`} preload="hover" viewTransition aria-label={a.name} className="absolute inset-0" />
      <div className="pointer-events-none relative aspect-square overflow-hidden" style={{ viewTransitionName: `img-${a.id}` }}>
        ...
        <div className="pointer-events-none absolute right-2 top-2 z-10 flex max-w-[85%] flex-wrap justify-end gap-1">
          {a.genres.slice(0, 2).map((g) => (
            <span key={g} className="rounded-full bg-black/45 px-2 py-0.5 text-[10px] font-medium text-white backdrop-blur-sm">
              {g}
            </span>
          ))}
        </div>
        ...
        <button
          onClick={toggle}
          aria-label={rowPlaying ? `Pause ${a.name}` : `Play ${a.name}`}
          className={`pointer-events-auto absolute left-1/2 top-1/2 z-10 grid h-16 w-16 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-text text-bg shadow-lg transition ${
      ...
      <div className="pointer-events-none flex items-center justify-between gap-2 p-3">
        <div className="min-w-0">
          <p className={`truncate text-sm font-semibold ${isCurrent ? "text-accent" : "text-text"}`} style={{ viewTransitionName: `name-${a.id}` }}>
            {a.name}
          </p>
          <p className="mt-0.5 truncate text-xs text-text-faint" style={{ viewTransitionName: `date-${a.id}` }}>
  ...
  return (
    <div className="px-4 py-5 sm:px-6 lg:px-8 lg:py-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="inline-flex shrink-0 self-start rounded-full border border-border p-0.5 text-xs">
          {VIEWS.map(([v, label]) => (
            <button
              key={v}
              type="button"
      ...
      {view === "grid" ? (
        ...
        <ul className={`mt-6 flex flex-col ${view === "compact" ? "gap-1.5" : "gap-2.5"}`}>
          {sorted.map((a) =>
            view === "compact" ? (
